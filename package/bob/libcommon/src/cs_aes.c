...
#if (AES_DEBUG == 1)
/*
static unsigned char key[(KEY_SIZE >> 3)] = "0123456789ABCDEF";
static unsigned char input[] = "Technology And Business Are Opposites!";
...
int main() {
  ...
  output = encode(&ctx, AES_ENCRYPT, key, input, 38);
  for (i = 0; i < 48; i++) { printf("0x%02X ", output[i]); }
  printf("\n");
  free(output);
...
*/
//gcc aes.c base64.c -o telecom -Wall
/*
static unsigned char key[(KEY_SIZE >> 3)] = "0123456789ABCDEF";
static unsigned char input[] = "Technology And Business Are Opposites!";
...
int main() {
  ...
  bindata = encode(&ctx, AES_ENCRYPT, key, input, 38);
  char base64[1024];

  memset(base64, 0, 1024);
  base64_encode(bindata, base64, encode_length(38));
  printf("%s\n", base64);

...
void
shttpd_memcpy(struct shttpd_arg *arg, void *source, int len)
{
	char		*buf = arg->out.buf + arg->out.num_bytes;
	int		buflen = arg->out.len - arg->out.num_bytes;
...
#if 0
int
match_regexp(const char *pattern, const char *string)
{
	if (*pattern == '\0')
		return (*string == '\0');
	...
	if (*pattern == '*')
		do {
			if (match_regexp(pattern + 1, string))
				return (1);
		} while (*string++ != '\0');

	if (*string != '\0' && *pattern == *string)
		return (match_regexp(pattern + 1, string + 1));
...
void
dump_registered_uri(struct shttpd_ctx *ctx)
{
#ifdef _DEBUG
	struct llhead		*lp;
	struct registered_uri	*reg_uri;
	...
	LL_FOREACH(&ctx->registered_uris, lp) {
		reg_uri = LL_ENTRY(lp, struct registered_uri, link);
		DBG(("%s %d: reg_uri->uri=%s", __FUNCTION__, __LINE__, reg_uri->uri));
	}
#endif /* _DEBUG */
}

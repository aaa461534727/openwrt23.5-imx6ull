...
{
#if 0
	if((stream->io_class == &_shttpd_io_file) ||(stream->io_class == &_shttpd_io_post))
		stream->conn->rem.flags |= FLAG_CLOSED;
#else
		...
		if (*p == '/') {
			*p = '\0';
			if (!_shttpd_stat(path, stp) && !S_ISDIR(stp->st_mode)) {
				c->path_info = p + 1;
				DBG(("%s %d: c->path_info=%s", __FUNCTION__, __LINE__, c->path_info));
				return (0);
			} else {
				*p = '/';
			}
...
/*
handle_request代替decide_what_to_do，不支持dir list，不支持alias
*/
static void
handle_request(struct conn *c)
{
	...
	if(c->query) {
		if ((c->query_buf = _shttpd_strndup(c->query, strlen(c->query))) == NULL) {
			send_error_async(500, "Bad Request", NULL, "Cannot allocate request", c);
			return;
		}

		init_cgi(&c->htab_get, c->query_buf);
		dump_htab(&c->htab_get);
	}

	_shttpd_url_decode(c->uri, strlen(c->uri), c->uri, strlen(c->uri) + 1);
	remove_double_dots(c->uri);

	if (strlen(c->uri) + strlen(c->ctx->options[OPT_ROOT]) >= URI_MAX) {
		send_error_async(400, "Bad Request", NULL, "URI is too long", c);
		return;
	}

	if(c->uri[0] != '/') {
		send_error_async(400, "Bad Request", NULL, "URI is invalid", c);
		return;
	}

	for (handler = mime_handlers; handler->pattern; handler++) {
		if (match_regexp(handler->pattern, c->uri))
			break;
	}

	
#if defined(SESSION_AUTH)
	if((handler->need_auth == 1)) {
		...
		if(strlen(password) > 0) {
			...
			if((login_state == 1)  &&  check_session_id(c->query)){
				c->auth_state = 1;
				update_login_data(client_ip);
			}else if(login_state == 2){
				//fix me,response ajax json
	...
	if (handler->do_response) {
		handler->do_response(c, handler);
	}
...
char *_shttpd_get_stream_type(struct stream *stream)
{
	return (stream == &stream->conn->loc) ? "loc" : "rem";
}
...
connection_desctructor(struct llhead *lp)
{
	...
	DBG(("Disconnecting %d do_close=%d", c->rem.chan.sock, do_close));
	...
	if (!do_close && c->loc.content_len > 0) {
		...
		c->query_buf = c->query = c->request = c->uri = c->path_info = NULL;
		c->mime_type.len = 0;
...
process_connection(struct conn *c, int remote_ready, int local_ready)
{
	...
	if (remote_ready && io_space_len(&c->rem.io))
		read_stream(&c->rem);											//从浏览器接收http请求数据
...
set_inetd(struct shttpd_ctx *ctx, const char *flag)
{
	if (_shttpd_is_true(flag)) {
		if (daemon(1, 1) == -1) {
			DBG(("%s %d: daemon: %m", __FUNCTION__, __LINE__));
			return (FALSE);
		}
	}
...
shttpd_init(int argc, char *argv[])
{
	...
	for (o = known_options; o->name != NULL; o++)
		ctx->options[o->index] = o->default_value ? _shttpd_strdup(o->default_value) : NULL;

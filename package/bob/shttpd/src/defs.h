...
#define TMP_UPLOAD_FILE "/tmp/uploaded.tmp"
#define FNL_UPLOAD_FILE "/tmp/uploaded.img"
...
extern char *_shttpd_get_stream_type(struct stream *stream);
...
extern const struct io_class	_shttpd_io_post;
extern const struct io_class	_shttpd_io_upload;
...
extern void dump_registered_uri(struct shttpd_ctx *ctx);

extern void	_shttpd_do_ssi(struct conn *);
extern void	_shttpd_ssi_func_destructor(struct llhead *lp);

extern int match_regexp(const char *pattern, const char *string);
